...
class MyTweetListener(tweepy.StreamListener):
    ...
    def on_connect(self):
        ...
        self.df = pd.DataFrame(
            columns=["Text", "Country", "Country Code", "Date"])

    # Method called every time an update is received
    def on_status(self, status):
        if not isinstance(status.place, type(None)):
            ...
            self.df = self.df[self.df["Date"] > current_time -
                              datetime.timedelta(seconds=minutes_erasing * 60)]

            self.dfByCountry = self.df.groupby("Country").sum(
            ).sort_values(by="No of Tweets", ascending=False)
